...
def define_CNN_model(slot, utterance_representations_full, num_filters=300, vector_dimension=300, longest_utterance_length=40):
    """
    ...
    pooled_outputs = []
    with tf.name_scope(slot.replace(" ", "_") + "_CNN"):
        for i, filter_size in enumerate(filter_sizes):
            #with tf.name_scope("conv-maxpool-%s" % filter_size):
            # Convolution Layer
            filter_shape = [filter_size, vector_dimension, 1, num_filters]
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name=slot.replace(" ", "_") + "_W")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name=slot.replace(" ", "_") + "_b")
            conv = tf.nn.conv2d(
                ...
                padding="VALID")
            # Apply nonlinearity
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name=slot.replace(" ", "_") + "_relu")
...
def model_definition(slot, vector_dimension, label_count, slot_vectors, value_vectors, use_delex_features=False, use_softmax=True, value_specific_decoder=False, learn_belief_state_update=True):
    """
    ...
    with tf.name_scope(slot.replace(" ", "_") + "_model"):
        with tf.name_scope(slot.replace(" ", "_") + "_model_declaration"):
            if use_softmax:
...
#           utterance_representations_delex = tf.placeholder(tf.float32, [None, label_size, 40, vector_dimension])
            ...
            W_slots = tf.constant(slot_vectors, name="W_init_slot")
            W_values = tf.constant(value_vectors, name="W_init_values")

            # output label, i.e. True / False, 1-hot encoded:
            y_ = tf.placeholder(tf.float32, [None, label_size], name="output_label")

            y_past_state = tf.placeholder(tf.float32, [None, label_size], name="past_output_label")
            ...
            # dstc8 version
            num_filters = 200
            h_utterance_representation = define_CNN_model(slot, utterance_representations_full, num_filters, vector_dimension, longest_utterance_length)

            w_candidates = tf.Variable(tf.random_normal([vector_dimension, vector_dimension]), name="candidates_weight")
            b_candidates = tf.Variable(tf.zeros([vector_dimension]), name="candidate_bias")
            ...
            w_joint_hidden_layer = tf.Variable(tf.random_normal([vector_dimension, hidden_units_1]), name="joint_hidden_layer_weight")
            b_joint_hidden_layer = tf.Variable(tf.zeros([hidden_units_1]), name="joint_hidden_layer_bias")
            ...
            w_joint_presoftmax = tf.Variable(tf.random_normal([hidden_units_1, 1]), name="joint_presoftmax_weight") # collapse to 1
            b_joint_presoftmax = tf.Variable(tf.zeros([1]), name="joint_presoftmax_bias") # collapse to 1
        ...
        with tf.name_scope(slot.replace(" ", "_") + "_system_request_network"):
        ...
        with tf.name_scope(slot.replace(" ", "_") + "_confirmation_network"):
        ...
        if use_softmax:
            ...
            with tf.name_scope(slot.replace(" ", "_") + "_presoftmax"):
                ...
                #y_presoftmax = d + mr +mc with dimension og
                y_presoftmax = y_presoftmax + sysconf + sysreq

        if use_delex_features:
            with tf.name_scope(slot.replace(" ", "_") + "_presoftmax_with_delex"):
    ...
    # value-specific decoder:
        ...
        if use_softmax:
            ...
            if learn_belief_state_update:
                ...
                if value_specific_decoder: # value-specific update
                    ...
                    with tf.name_scope(slot.replace(" ", "_") + "_softmax_learn_belief_state_update"):
                        update_coefficient = tf.constant(0.7)

                        a_memory = tf.Variable(tf.random_normal([1, 1]), name=slot.replace(" ", "_") + "_a_memory")
                        diag_memory = a_memory * tf.diag(tf.ones(label_size))

                        b_memory = tf.Variable(tf.random_normal([1, 1]), name=slot.replace(" ", "_") + "_b_memory")
                        ...
                        a_current = tf.Variable(tf.random_normal([1, 1]), name=slot.replace(" ", "_") + "_a_current")
        ...
        if use_softmax:
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=y_combine, labels=y_, name="loss_softmax_cross_entropy")
        else:
            cross_entropy = tf.reduce_sum(tf.square(y - y_), name="loss_cross_entropy")
