...
def process_dstc8_dialogue(dstc8_dialogue, slots, service_name):
    # assume only consider single service
    service_name = service_name.lower()
    null_bs = {}
    for slot in slots:
        if slot.endswith("_request"):
            null_bs[slot] = []
        else:
            null_bs[slot] = "none"

    informable_slots = slots
    pure_requestables = slots
    ...
    for idx, turn in enumerate(dstc8_dialogue):
        if idx % 2 == 0:
            ...
            for requested_slot in current_labels["requested_slots"]:
                ...
                current_bs[request_slot_name].append(full_slot_name)

            # add active intent slot to current_bs
            active_intent_slot_name = service_name + "_" + "active_intent"
            active_intent = current_labels["active_intent"]
            if active_intent == "NONE":
                active_intent = active_intent.lower()
            current_bs[active_intent_slot_name] = service_name + "_" + active_intent
...
def load_dstc8_data_file(file_path, dialogue_ontology, dialogue_number_limit=10000000000000000000000):
    ...
    for idx in range(0, dialogue_count):
        if idx >= dialogue_number_limit:
            break
        # assume only single domain
        service_name = dstc8_json[idx]["services"][0]
        # process each dialogues
        current_dialogue = process_dstc8_dialogue(dstc8_json[idx]["turns"], dialogue_ontology.keys(), service_name)
        ...
        for turn_idx, turn in enumerate(current_dialogue):
            ...
            requested_slot_name = service_name + "_" + "request"
            requested_slot_name = requested_slot_name.lower()
            for req_slot in turn[4][requested_slot_name]:
                current_label.append((requested_slot_name, req_slot))
...
def track_dialogue_dstc8(model_variables, word_vectors, dialogue_ontology, dstc8_dialogue, sessions):
    ...
    for idx, trans_and_req_and_label_and_currlabel in enumerate(dstc8_dialogue):
        ...
        for slot in slots_to_track:
            ...
            if idx == 0 or slot.endswith("_request"):
...
def delexicalise_utterance_values(utterance, target_slot, target_values):
    ...
    # dstc8 version
    if target_slot.endswith("_request"):
...
def generate_examples(target_slot, feature_vectors, word_vectors, dialogue_ontology,
    ...
    if pos_example_count == 0 or positive_count == 0 or negative_count == 0 or neg_example_count == 0:
        print("#### SKIPPING (NO DATA): ", target_slot, pos_example_count, positive_count, neg_example_count,
              negative_count)
        return None
...
def evaluate_model(dataset_name, sess, model_variables, data, target_slot, utterances, dialogue_ontology, \
    ...
    if print_mode:
        print("Epoch", epoch_id, "[Accuracy] = ", eval_accuracy, " ----- update coeff:",
              update_coefficient_load)  # , round(end_time - start_time, 1), "seconds. ---"
...
def load_word_vectors_dstc8(word_vector_destination, index_destination):
    print("Loading pretrained word vectors from", word_vector_destination, "and ", index_destination,
          "as index dictionary.")
    word_dictionary = {}
    ...
    for line in f:
        ...
        if language_key in transformed_key:
            ...
            try:
                ...
                print(
                    'DEBUG: Exception message (FILE={0:s}, LINE={1:d}): {2:s}'.format(fname, exc_tb.tb_lineno, str(e)))
                ...
                for word_idx, word in enumerate(words_utterance):
                    ...
                    try:
                        ...
                        print("Something off with word:", word, word in word_vectors,
                              numpy.array(word_vectors[word]).shape)
...
def divide_train_validate_data(whole_feature_vectors, whole_positive_examples, whole_negative_examples,
                               train_ratio=0.8):
    # for idx_example, example in enumerate(examples):
    #
    #     (utterance_idx, utterance, value_idx) = example
...
def train_run_dstc8(target_language, override_en_ontology, percentage, model_type, dataset_name, word_vectors, exp_name,
                    dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20,
                    batches_per_epoch=4096, batch_size=256):
    """
    This method trains a model on the data and saves the file parameters to a file which can
    ...
    # dstc8 version
    _, utterances_train_validate = load_dstc8_data("data/" + dataset_name + "/train", dialogue_ontology,
                                                   last_index=1)  # , last_index=1, dialogue_number_limit=1
    ...
    # dstc8 version
    # _, utterances_val2 = load_dstc8_data("data/" + dataset_name + "/validate", dialogue_ontology , last_index=3) # , utterances_val2 = load_dstc8_data("data/" + dataset_name + "/validate", dialogue_ontology) #
    #
    # val_count = len(utterances_val2)
    ...
    whole_feature_vectors, whole_positive_examples, whole_negative_examples = generate_data(utterances_train_validate,
                                                                                            word_vectors,
                                                                                            dialogue_ontology,
                                                                                            target_slot)
    ...
    while epoch < max_epoch:
        ...
        if current_metric > best_f_score:
            ...
            # dstc8 version
            path_to_save = "./models/dstc8/" + model_type + str(dataset_name) + str(target_slot) + "_" + str(
                exp_name) + "_" + str(percentage) + ".ckpt"
...
def purify_ontology(dialogue_ontology, service_number=None):
    ...
    for service in dialogue_ontology:
        ...
        slot_full_description[
            active_intent_slot_name.lower()] = service_description + "track currently active user intents"
...
def load_request_intent_ontology(dialogue_ontology):
    ...
    for service in dialogue_ontology:
        ...
        for intent in service_intents:
            ...
            intent_description[intent_name] = service_description + ' ' + intent['description']

    request_intent_ontology['request'] = requested_slot_names
    request_intent_ontology['active_inten'] = intent_names
...
class NeuralBeliefTracker:
    ...
    def __init__(self, config_filepath):
        ...
        request_intent_ontology, requested_slot_description, intent_description = load_request_intent_ontology(
            dialogue_ontology)

        slots = dialogue_ontology.keys()

        sample_word_vector = random.sample(list(word_vectors.values()), 1)
        # print(numpy.shape(sample_word_vector))
        ...
        for slot_name in slots:
            if dontcare_value not in dialogue_ontology[slot_name] and not slot_name.endswith(
                    "_request") and not slot_name.endswith("_active_intent"):
                dialogue_ontology[slot_name].append(dontcare_value)
            for value in dialogue_ontology[slot_name]:
                value = str(value)
                if slot_name.endswith("_active_intent"):
                    continue
                if " " not in value and value not in word_vectors and "_" not in value:
                    # original version
                    # word_vectors[str(value)] = xavier_vector(str(value))
                    # dstc8 version
                    word_vectors[str(value)] = xavier_vector(str(value), D=200)
                    print("-- Generating word vector for:", value.encode("utf-8"), ":::",
                          numpy.sum(word_vectors[value]))
        ...
        # dstc8 version
        for slot in dialogue_ontology.keys():
            slot_description = slot_full_description[slot]
            word_vectors[slot.lower()] = numpy.zeros((word_vector_size,), dtype="float32")
            constituent_words = slot_description.split()
            count = 0.0
            for word in constituent_words:
                word = str(word)
                if word in word_vectors:
                    count += 1.0
                    word_vectors[slot.lower()] += word_vectors[word]
            word_vectors[slot.lower()] = [x / count for x in word_vectors[slot.lower()]]

            for value in dialogue_ontology[slot]:
                print(value)
                if slot.endswith('_active_intent'):
                    value = str(value)
                    ...
                    if len(constituent_words) == 1:
                        constituent_words = value.split('_')
                    count = 0.0
                    for word in constituent_words:
                        word = str(word)
                        if word in word_vectors:
        ...
        for slot in dialogue_ontology:
            # dstc8 version
            if not slot.startswith("restaurants_1_request"):
                continue
            ...
            if slot.endswith("_request"):
                ...
                self.model_variables[slot] = model_definition(slot, word_vector_size, len(dialogue_ontology[slot]),
                ...
                # dstc8 version
                slot_vectors = numpy.zeros((len(dialogue_ontology[slot]) + 1, 200), dtype="float32")  # +1 for None
                value_vectors = numpy.zeros((len(dialogue_ontology[slot]) + 1, 200), dtype="float32")

                for value_idx, value in enumerate(dialogue_ontology[slot]):
                    slot_vectors[value_idx, :] = word_vectors[slot.lower()]
                    value_vectors[value_idx, :] = word_vectors[value]
        ...
        for slot in self.dialogue_ontology.keys():
            ...
            if dataset == "woz":
                train_run(target_language=self.language, override_en_ontology=False, percentage=1.0, model_type="CNN",
                          ...
                          word_vectors=self.word_vectors, exp_name=self.exp_name,
                          dialogue_ontology=self.dialogue_ontology, model_variables=self.model_variables[slot],
                          target_slot=slot, language=self.language_suffix, \
                          max_epoch=self.max_epoch, batches_per_epoch=self.batches_per_epoch,
                          batch_size=self.batch_size)
            elif dataset == "dstc8":
                train_run_dstc8(target_language=self.language, override_en_ontology=False, percentage=1.0,
                                model_type="CNN", dataset_name=self.dataset_name, \
                                word_vectors=self.word_vectors, exp_name=self.exp_name,
                                dialogue_ontology=self.dialogue_ontology, model_variables=self.model_variables[slot],
                                target_slot=slot, language=self.language_suffix, \
                                max_epoch=self.max_epoch, batches_per_epoch=self.batches_per_epoch,
    ...
    def test_dstc8(self):
        ...
        for model_id in range(0, self.num_models):
            ...
            for load_slot in slots_to_load:
                path_to_load = "./models/dstc8/" + self.model_type + str(self.dataset_name) + str(
                    load_slot) + "_" + str(exp_name) + "_" + str(
                    percentage) + ".ckpt"
            ...
            evaluated_dialogues, belief_states = track_dstc8_data(dstc8_dialogues, self.model_variables,
                                                                  self.word_vectors,
                                                                  self.dialogue_ontology, sessions)
            list_of_belief_states.append(belief_states)  # only useful for interpolating.
